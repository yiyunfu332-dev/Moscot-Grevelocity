...
md='''# MOSCOT project: professor sharing package
...
1. [Complete illustrated figure report](MOSCOT_project_figures_for_professor.pdf): six new overview charts/pages drawn from saved results, followed by the historical/current figure appendix. Every page states its evidence status. The overview pages are newly drawn; the underlying fits are not new.
...
|---|---:|---:|---|
'''
for n in notebooks:md+=f"| [{Path(n['path']).name}]({n['path']}) | {n['executed_cells']}/{n['nonempty_code_cells']} | {len(n['errors'])} | {n['status']} |\n"
md+='''
